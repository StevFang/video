...
public class FFmpegOnlineCommandServiceImpl implements CommandService<FfmpegLiveConfig> {

    private static Logger logger = LoggerFactory.getLogger(FFmpegOnlineCommandServiceImpl.class);

    @Override
    public String createCommand(FfmpegLiveConfig ffmpegLiveConfig) {
        try{
            String ffmpegPath = ffmpegLiveConfig.getFfmpegPath();
            if(StringUtils.isNotBlank(ffmpegPath)) {
                ...
                String input = ffmpegLiveConfig.getInput();
                String output = ffmpegLiveConfig.getOutput();
                String appName = ffmpegLiveConfig.getAppName();
                String twoPart = ffmpegLiveConfig.getTwoPart();
                String codec = ffmpegLiveConfig.getCodec();
