...
@Slf4j
@Service
public class FfmpegDecodeCommandServiceImpl implements CommandService<DecodeffmpegDTO> {

    @Override
    public String createCommand(DecodeffmpegDTO config) {
     ...
     * @return
     */
    private String processAVI(DecodeffmpegDTO config) {
     ...
     * @return
     */
    private String getFastForwardMPEGCommand(DecodeffmpegDTO config) {
        String sourcePath = config.getSourcePath();
        try{
            StringBuilder command = new StringBuilder();
