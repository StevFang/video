...
@Slf4j
@Service
public class FfmpegOnlineCommandServiceImpl implements CommandService<FastForwardMovingPictureExpertsGroupLiveConfig> {

    @Override
    public String createCommand(FastForwardMovingPictureExpertsGroupLiveConfig fastForwardMovingPictureExpertsGroupLiveConfigDTOConfig) {
        try{
            ...
            if(StringUtils.isNotBlank(fastForwardMPEGPath)) {
                ...
                String input = fastForwardMovingPictureExpertsGroupLiveConfigDTOConfig.getInput();
                String output = fastForwardMovingPictureExpertsGroupLiveConfigDTOConfig.getOutput();
                String appName = fastForwardMovingPictureExpertsGroupLiveConfigDTOConfig.getAppName();
                String twoPart = fastForwardMovingPictureExpertsGroupLiveConfigDTOConfig.getTwoPart();
                String codec = fastForwardMovingPictureExpertsGroupLiveConfigDTOConfig.getCodec();
                ...
                if (CommonConstant.TWO_PART_0.equals(twoPart)) {
                    ...
                    // -f ：转换格式，默认flv
                    String fmt = fastForwardMovingPictureExpertsGroupLiveConfigDTOConfig.getFmt();
                    if(StringUtils.isNotBlank(fmt)){
                        command.append(" -f ").append(fmt);
                    }
                    // -r :帧率，默认25；-g :帧间隔
                    String fps = fastForwardMovingPictureExpertsGroupLiveConfigDTOConfig.getFps();
                    if (StringUtils.isNotBlank(fps)) {
                        command.append(" -r ").append(fps);
                        command.append(" -g ").append(fps);
                    }
                    // -s 分辨率 默认是原分辨率
                    String rs = fastForwardMovingPictureExpertsGroupLiveConfigDTOConfig.getRs();
